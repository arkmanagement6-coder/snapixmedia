  ...
  return (
    <div className="relative min-h-screen bg-transparent text-slate-300 overflow-x-hidden flex flex-col font-sans">
      ...
      <section className="relative pt-36 pb-12 px-6 glow-grid">
        <div className="absolute top-1/4 left-1/4 w-[400px] h-[300px] bg-radial from-neon-purple/10 to-transparent blur-[80px] pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-900/40 border border-white/5 shadow-md">
                ...
                <div>
                  ...
                  <a href="mailto:snapixmediaofficial@gmail.com" className="block text-sm text-slate-200 hover:text-white font-bold mt-0.5">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-900/40 border border-white/5 shadow-md">
                ...
                <div>
                  ...
                  <a href="tel:+919675818088" className="block text-sm text-slate-200 hover:text-white font-bold mt-0.5">
                    +91 9675818088
                  </a>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-900/40 border border-white/5 shadow-md">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 font-bold leading-relaxed mt-0.5">
            ...
            {/* WhatsApp CTA Call */}
            <GlassCard className="flex flex-col gap-4 border-emerald-500/15 hover:border-emerald-500/30 bg-slate-900/40" hoverGlow>
            ...
            <div className="rounded-3xl border border-white/5 bg-slate-950 h-44 overflow-hidden relative group">
              <div className="absolute inset-0 bg-[#090514] opacity-90" />
              ...
              <div className="absolute top-1/2 left-1/3 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                ...
                <span className="text-[9px] font-bold font-mono px-2 py-0.5 rounded bg-slate-950 text-white mt-1 border border-white/10 shadow-sm">Snapix Media Agra</span>
              </div>
            </div>
          </div>
            ...
            <GlassCard className="p-6 sm:p-8" hoverGlow>
              {formSubmitted ? (
                ...
                <form onSubmit={handleContactSubmit} className="flex flex-col gap-5">
                  <h3 className="text-lg font-extrabold text-white font-display border-b border-white/5 pb-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-neon-blue"
                        />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs text-slate-400 font-bold uppercase">Interest Domain</label>
                      <select
                        value={formData.service}
                        onChange={(e) => setFormData({ ...formData, service: e.target.value })}
                        className="w-full px-4 py-3 bg-slate-900 border border-white/10 text-white rounded-xl text-xs focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/30 transition-all"
                      >
                        <option value="web" className="bg-slate-950 text-white">Web Engineering</option>
                        <option value="seo" className="bg-slate-950 text-white">SEO Optimization</option>
                        <option value="marketing" className="bg-slate-950 text-white">Digital Marketing</option>
                        <option value="design" className="bg-slate-950 text-white">UI/UX Architecture</option>
                        <option value="other" className="bg-slate-950 text-white">Multiple / Full Retainer</option>
                      </select>
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs text-slate-400 font-bold uppercase">Budget Range</label>
                      <select
                        value={formData.budget}
                        onChange={(e) => setFormData({ ...formData, budget: e.target.value })}
                        className="w-full px-4 py-3 bg-slate-900 border border-white/10 text-white rounded-xl text-xs focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/30 transition-all"
                      >
                        <option value="$5k-$10k" className="bg-slate-950 text-white">$5k - $10k</option>
                        <option value="$10k-$25k" className="bg-slate-950 text-white">$10k - $25k</option>
                        <option value="$25k-$50k" className="bg-slate-950 text-white">$25k - $50k</option>
                        <option value="$50k+" className="bg-slate-950 text-white">$50k+ Enterprise</option>
                      </select>
                    </div>
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs text-slate-400 font-bold uppercase">Brief Message</label>
                    <textarea
                      placeholder="Outline your systems or campaigns constraints..."
                      rows={4}
                      value={formData.message}
                      onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                      className="w-full px-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/30 transition-all resize-none"
                    />
                  </div>
              ...
              {schedulerSubmitted ? (
                ...
                <form onSubmit={handleBookingSubmit} className="flex flex-col gap-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-2 font-semibold">Pick a day and time slot to sync with our strategic advisor.</p>
                  </div>
                        ...
                        <button
                          ...
                          className={`p-1.5 sm:p-3 rounded-lg sm:rounded-xl border flex flex-col items-center justify-center gap-1 cursor-pointer transition-all duration-300 ${
                            selectedDay === day.date
                              ...
                              : "bg-slate-900/60 border-white/10 text-slate-300 hover:text-white hover:border-neon-purple/40"
                          }`}
                        >
