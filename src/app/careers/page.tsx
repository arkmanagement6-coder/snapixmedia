  ...
  return (
    <div className="relative min-h-screen bg-transparent text-slate-300 overflow-x-hidden flex flex-col font-sans">
      ...
      <section className="relative pt-36 pb-16 px-6 glow-grid">
        <div className="absolute top-1/4 left-1/2 w-[400px] h-[300px] bg-radial from-neon-cyan/10 to-transparent blur-[80px] pointer-events-none -translate-x-1/2" />
      ...
      {/* Benefits Section */}
      <section className="py-16 px-6 border-y border-white/5 bg-slate-950/20">
            ...
            {benefits.map((perk, idx) => {
              ...
              return (
                <GlassCard key={idx} className="flex flex-col gap-4" hoverGlow>
                  <div className={`w-10 h-10 rounded-xl border flex items-center justify-center border-white/5 ${perk.color}`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <h3 className="text-base font-bold text-white">{perk.title}</h3>
                  <p className="text-xs text-slate-400 leading-relaxed font-medium">{perk.desc}</p>
                </GlassCard>
              );
            })}
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col gap-6">
            {openings.map((job, idx) => (
              <div
                key={idx}
                className="group p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-neon-purple/45 hover:bg-slate-900/60 hover:shadow-[0_10px_25px_-5px_rgba(124,58,237,0.15)] transition-all duration-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-md"
              >
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-slate-400 font-semibold mt-1">
                    ...
                    <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-slate-500" /> {job.type}</span>
                  </div>
                </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-fade-in font-sans">
          <div className="w-full max-w-lg rounded-3xl bg-slate-950 border border-white/10 p-6 sm:p-8 relative shadow-2xl">
            <button
              onClick={() => setSelectedJob(null)}
              className="absolute top-4 right-4 p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-900 transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            {submitted ? (
              <div className="flex flex-col items-center justify-center text-center py-12 gap-4">
                ...
                <p className="text-xs text-slate-400 leading-relaxed max-w-xs font-semibold">
                  Thanks for applying for the <strong className="text-white">{selectedJob}</strong> position. Our engineering and recruitment recruitment panel will contact you soon.
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1 font-semibold">Applying for: <strong className="text-neon-purple">{selectedJob}</strong></p>
                </div>
                ...
                <form onSubmit={handleApply} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/30 transition-all"
                      />
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs text-slate-400 font-bold uppercase">Resume Link / URL</label>
                    <input
                      type="url"
                      ...
                      className="w-full px-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/30 transition-all"
                    />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs text-slate-400 font-bold uppercase">Cover Letter / Notes (Optional)</label>
                    <textarea
