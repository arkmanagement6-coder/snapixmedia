  ...
  return (
    <div className="relative min-h-screen bg-[#030014] text-[#ededed] overflow-x-hidden flex flex-col font-sans">
      ...
      <section className="relative pt-36 pb-20 px-6 glow-grid">
        <div className="absolute top-1/3 left-1/2 w-[500px] h-[300px] bg-radial from-neon-purple/8 to-transparent blur-[100px] pointer-events-none -translate-x-1/2" />
      ...
      <section className="py-20 px-6 border-t border-glass-border bg-space-black/20">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-xl mx-auto mb-16">
            ...
            <p className="text-gray-400 text-sm">A collective of developers, visual designers, and growth hackers.</p>
          </div>
            ...
            {teamMembers.map((member, i) => (
              <GlassCard key={i} className="flex flex-col justify-between text-center items-center h-[340px]" hoverGlow>
                <div className="flex flex-col items-center gap-4">
                  <div className={`w-20 h-20 rounded-full bg-gradient-to-tr ${member.gradient} flex items-center justify-center font-bold text-black text-2xl shadow-lg`}>
                    {member.initials}
                  </div>
                  <div>
                  ...
                  <p className="text-xs text-gray-400 leading-relaxed mt-2">{member.bio}</p>
                </div>
                
                <div className="flex gap-4 mt-6">
                  {["LN", "GH"].map((social) => (
                    <span key={social} className="text-[10px] font-mono text-gray-500 hover:text-white cursor-pointer transition-colors duration-300">
                      {social}
                    </span>
                  ))}
                </div>
          ...
          <div className="relative border-l border-glass-border/40 pl-8 ml-4 flex flex-col gap-12">
            {milestones.map((item, i) => (
              <div key={i} className="relative">
                {/* Timeline node */}
                <span className="absolute -left-[41px] top-1.5 w-[18px] h-[18px] rounded-full bg-space-dark border-2 border-neon-cyan flex items-center justify-center">
