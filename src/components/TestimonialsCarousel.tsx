...
export default function TestimonialsCarousel() {
  ...
  return (
    <div className="relative max-w-3xl mx-auto px-4 py-8 font-sans">
      <Quote className="absolute top-0 left-0 w-24 h-24 text-white/3 pointer-events-none -translate-x-6 -translate-y-8" />
      
      <GlassCard className="p-8 md:p-12 relative overflow-hidden bg-glass-card border border-glass-border rounded-3xl min-h-[280px] flex flex-col justify-between" hoverGlow>
        <div className="relative overflow-hidden flex-1">
          <AnimatePresence initial={false} custom={direction} mode="wait">
            ...
            >
              ...
              {/* Review Text */}
              <p className="text-base sm:text-lg md:text-xl text-gray-200 leading-relaxed font-medium italic">
                "{current.quote}"
              </p>

              {/* Author Info */}
              <div className="flex items-center gap-4 mt-2">
                <div className={`w-12 h-12 rounded-full bg-gradient-to-tr ${current.gradient} flex items-center justify-center font-bold text-black text-sm tracking-wide shadow-md`}>
                  {current.initials}
                </div>
                <div>
        ...
        {/* Navigation Arrows */}
        <div className="flex justify-end gap-3 mt-8 border-t border-glass-border/30 pt-6">
          <button
            onClick={handlePrev}
            className="w-10 h-10 flex items-center justify-center rounded-full border border-glass-border bg-glass-card hover:bg-white hover:text-black hover:border-white transition-all duration-300 cursor-pointer"
