...
export default function TestimonialsCarousel() {
  ...
  return (
    <div className="relative max-w-3xl mx-auto px-4 py-8 font-sans">
      <Quote className="absolute top-0 left-0 w-24 h-24 text-slate-200 pointer-events-none -translate-x-6 -translate-y-8 opacity-40 animate-pulse" />
      
      <GlassCard className="p-8 md:p-12 relative overflow-hidden rounded-3xl min-h-[280px] flex flex-col justify-between shadow-sm" hoverGlow>
          ...
          <AnimatePresence initial={false} custom={direction} mode="wait">
            ...
            >
              ...
              <div className="flex items-center gap-4 mt-2">
                <div className={`w-12 h-12 rounded-full bg-gradient-to-tr ${current.gradient} flex items-center justify-center font-bold text-white text-sm tracking-wide shadow-sm`}>
                  {current.initials}
                </div>
                <div>
        ...
        <div className="flex justify-end gap-3 mt-8 border-t border-slate-200/50 pt-6">
          <button
            onClick={handlePrev}
            className="w-10 h-10 flex items-center justify-center rounded-full border border-slate-200 bg-white text-slate-650 hover:bg-slate-50 hover:text-slate-900 hover:border-neon-cyan transition-all duration-300 cursor-pointer shadow-sm"
