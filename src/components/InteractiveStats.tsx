...
function StatCounter({ value, suffix, label, icon: Icon, glowColor, textColor, iconColor }: StatItemProps) {
  ...
  return (
    <motion.div
      ...
      className="relative flex flex-col items-center justify-center p-6 sm:p-8 rounded-3xl bg-white/90 border border-slate-200/80 group hover:border-blue-500/40 hover:bg-white transition-all duration-300 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.08)] hover:shadow-2xl hover:shadow-blue-500/10"
    >
      {/* Decorative vertical gradient bar on hover */}
      <span className="absolute left-0 top-1/4 bottom-1/4 w-[3px] rounded-r bg-gradient-to-b from-[#1A50F1] to-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
...
export default function InteractiveStats() {
  const stats = [
    { 
      ...
      icon: Rocket,
      glowColor: "rgba(26,80,241,0.08)",
      textColor: "text-[#1A50F1]",
      iconColor: "text-[#1A50F1] border-blue-100"
    },
    { 
      value: 100, 
      suffix: "+", 
